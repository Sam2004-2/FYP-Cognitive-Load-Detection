...
from src.cle.train.evaluation import (
    aggregate_to_session,
    compute_physio_validation,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Target column for training (default: load_0_1)",
    )
    parser.add_argument(
        "--physio-labels",
        type=str,
        default=None,
        help="Path to physio_stress_labels.csv for independent physiological validation",
    )
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    target_column = args.target_col or config.get("target.column", "load_0_1")
    logger.info(f"Target column: {target_column}")
    ...
    # Skip when training target is already physio_stress_score
    physio_labels_path = args.physio_labels or config.get(
        "physio_validation.labels_path", None
    )
    physio_validation_results = None
    if physio_labels_path and target_column != "physio_stress_score":
        logger.info("\n" + "=" * 80)
        logger.info("PHYSIOLOGICAL VALIDATION (independent secondary metric)")
        logger.info("=" * 80)
        logger.info(
            "Correlating model predictions with within-subject physio arousal"
        )
        physio_validation_results = compute_physio_validation(
            cv_results["predictions_df"],
            physio_labels_path,
        )
        if physio_validation_results.get("status") == "computed":
            pv = physio_validation_results
            logger.info(f"  Matched windows: {pv['n_matched_windows']}")
            logger.info(f"  Users with correlation: {pv['n_users_with_correlation']}")
            logger.info(f"  Overall Spearman r: {pv['overall_spearman_r']}")
            logger.info(
                f"  Mean within-subject Spearman: "
                f"{pv['mean_within_subject_spearman']}"
            )
        else:
            logger.warning(
                f"Physio validation skipped: {physio_validation_results.get('status')}"
            )
    else:
        logger.info(
            "Physio validation skipped (no --physio-labels or config path provided)"
        )
    ...
    # Include physio validation if available
    if physio_validation_results is not None:
        cv_results_to_save["physio_validation"] = physio_validation_results
